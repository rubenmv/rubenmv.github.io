...
(function(window) {
	...
	{	
		...
		{
			if (goLeft) this.xspeed -= ACCEL;
			if (goRight) this.xspeed += ACCEL;
		}
		...
		else this.xspeed *= FRIC;
		
		// Aplicamos la posicion final, teniendo cuenta los limites
		if (this.x < this.width / 2)	{
			this.x = this.width / 2; this.xspeed = 0;
		}
		else if (this.x > stage.canvas.width - this.width / 2) {
			this.x = stage.canvas.width - this.width / 2; this.xspeed = 0;
		}
		else {
			this.x += this.xspeed;
		}
